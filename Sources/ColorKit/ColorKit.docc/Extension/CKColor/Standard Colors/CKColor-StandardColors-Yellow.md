...
@Metadata {
    ...
    @DisplayName("Yellow", style: symbol)
...
A standard yellow color that adapts to the system environment.

## Overview

`CKColor.yellow` provides a platform-integrated yellow color. It automatically resolves to the appropriate system variant (such as `systemYellow` on iOS or macOS).
...
- **Gamut:** sRGB
- **Fallback Value:** RGB(1.0, 1.0, 0.0)
